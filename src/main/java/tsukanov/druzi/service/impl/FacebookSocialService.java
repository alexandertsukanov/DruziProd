...
import tsukanov.druzi.service.SocialService;
...
public class FacebookSocialService extends AbstractCreateProfileService implements SocialService<User> {
	...
	protected void extractAccountAvatar(User model, Profile profile) {
		try {
			String link = String.format("https://graph.facebook.com/%s/picture?width=%s&height=%s", model.getId(), Constants.UIImageType.AVATARS.getLargeWidth(), Constants.UIImageType.AVATARS.getLargeHeight());
