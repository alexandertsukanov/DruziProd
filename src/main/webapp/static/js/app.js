var druzi = {
	...
	createCertificateUploader : function(csrfToken){
		...
		$('#certificateFile').on('fileuploaderror', function(event, data, msg) {
		    druzi.showErrorDialog(messages.errorUploadCertificate);
		});
	},
	...
	moreProfiles : function(searchQuery) {
		...
		$.ajax({
			...
			error : function(data) {
				druzi.showErrorDialog(messages.errorAjax);
			}
		});
	},
	...
	ui : {
		...
		getTemplate : function() {
			if (druzi.ui.template == null) {
				var source = $("#ui-block-template").html();
				druzi.ui.template = Handlebars.compile(source);
			}
			return druzi.ui.template;
		},

		addBlock : function() {
			var template = druzi.ui.getTemplate();
		...
		save : function() {
			...
			if (selectedHobbyButtons.length > maxHobbies) {
				var closeFunction = function() {
					...
					if (druzi.hobbies.errorTimeout != null) {
						clearTimeout(druzi.hobbies.errorTimeout);
						druzi.hobbies.errorTimeout = null;
					}
				};
				$('#errorAlert button').unbind('click');
				$('#errorAlert button').click(closeFunction);
				$('#errorAlert').fadeIn('slow');
				druzi.hobbies.errorTimeout = setTimeout(closeFunction, 5000);
				return;
			}
