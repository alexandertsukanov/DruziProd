...
<druzi:edit-tab-header selected="profile" />
...
<div class="panel panel-default edit-profile">
	<div class="panel-body">
		...
		<druzi:form-display-error-if-invalid formName="contactsForm" />
		<form:form commandName="profileForm" action="/edit?${_csrf.parameterName}=${_csrf.token}" method="post" cssClass="form-horizontal data-form" enctype="multipart/form-data">
			...
			<druzi:form-has-error path="profilePhoto" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
				...
				<div class="col-sm-5">
					...
					<druzi:form-error path="profilePhoto" />
				</div>
			...
			<druzi:form-has-error path="birthDay" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
			...
			<div class="form-group ${hasError ? 'has-error' : ''}">
				...
				<div class="col-sm-5">
					...
					<druzi:form-error path="country" />
				</div>
				<div class="col-sm-5 help-block"></div>
			</div>
			<druzi:form-has-error path="city" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
				...
				<div class="col-sm-5">
					...
					<druzi:form-error path="city" />
				</div>
				<div class="col-sm-5 help-block"></div>
			</div>
			<druzi:form-has-error path="email" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
			...
			<druzi:form-has-error path="phone" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
			...
			<druzi:form-has-error path="objective" />
			<div class="form-group ${hasError ? 'has-error' : ''}">
			...
			<div class="form-group ${hasError ? 'has-error' : ''}">
				...
				<div class="col-sm-5">
					<form:textarea path="summary" class="form-control" id="inputSummary" required="required" rows="7"
						placeholder="Example: Two Java professional courses with developing two web applications: blog and druzi (Links to demo are provided)" />
					<druzi:form-error path="summary" />
				</div>
				<div class="col-sm-5 help-block">
					<blockquote>
...
$(document).ready(function(){
	druzi.createDatePicker();
	druzi.createPhotoUploader();
});
</script>
</content>