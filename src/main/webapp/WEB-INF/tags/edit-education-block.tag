...
<%@ attribute name="education" required="false" type="tsukanov.druzi.entity.Education"%>
...
<div id="ui-item-${index }" class="panel panel-default">
	...
	<div class="panel-body ui-item">
		...
		<div class="row">
			<c:if test="${education != null }"><druzi:form-has-error path="items[${index }].summary"/></c:if>
			<div class="col-xs-12 col-md-12 ${hasError ? 'has-error has-feedback' : ''}">
				...
				<druzi:form-error path="items[${index }].summary" />
				</c:if>
			</div>
			...
			<c:if test="${education != null }"><druzi:form-has-error path="items[${index }].finishYear"/></c:if>
			<div class="col-xs-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${education != null }">
				<druzi:form-error path="items[${index }].finishYear" displayIcon="false"/>
				</c:if>
			</div>
			<c:if test="${education != null }"><druzi:form-has-error path="items[${index }].university"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${education != null }">
				<druzi:form-error path="items[${index }].university" />
				</c:if>
			</div>
			<c:if test="${education != null }"><druzi:form-has-error path="items[${index }].faculty"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${education != null }">
				<druzi:form-error path="items[${index }].faculty" />
				</c:if>
			</div>
		</div>
	</div>
