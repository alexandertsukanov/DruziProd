...
<%@ attribute name="course" required="false" type="tsukanov.druzi.entity.Course"%>
...
<div id="ui-item-${index }" class="panel panel-default">
	...
	<div class="panel-body ui-item">
		...
		<div class="row">
			<c:if test="${course != null }"><druzi:form-has-error path="items[${index }].name"/></c:if>
			<div class="col-xs-6 col-md-4 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<druzi:form-error path="items[${index }].name" />
				</c:if>
			</div>
			<c:if test="${course != null }"><druzi:form-has-error path="items[${index }].school"/></c:if>
			<div class="col-xs-6 col-md-4 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${course != null }">
				<druzi:form-error path="items[${index }].school" />
				</c:if>
			</div>
