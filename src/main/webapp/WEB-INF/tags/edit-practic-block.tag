...
<%@ attribute name="practic" required="false" type="tsukanov.druzi.entity.Practic"%>
...
<div id="ui-item-${index }" class="panel panel-default">
	...
	<div class="panel-body ui-item">
		...
		<div class="row">
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].position"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<druzi:form-error path="items[${index }].position" />
				</c:if>
			</div>
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].company"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${practic != null }">
				<druzi:form-error path="items[${index }].company" />
				</c:if>
			</div>
			...
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].finishDate"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				<label class="control-label" for="items${index }finishDate">Дата завершения</label> 
				<div class="row">
					<div class="col-xs-6">
						<select id="items${index }finishDateMonth" name="items[${index }].finishDateMonth" class="form-control" onchange="druzi.ui.updateSelect($(this))" data-ref-select="items${index }finishDateYear">
					...
					<div class="col-xs-6">
						<select id="items${index }finishDateYear" name="items[${index }].finishDateYear" class="form-control" onchange="druzi.ui.updateSelect($(this))" data-ref-select="items${index }finishDateMonth">
						<option value="">Not finished yet</option>
						<c:forEach var="year" items="${years }">
				...
				<c:if test="${practic != null }">
				<druzi:form-error path="items[${index }].finishDate" displayIcon="false" />
				</c:if>
			</div>
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].responsibilities"/></c:if>
			<div class="col-xs-12 col-md-12 ${hasError ? 'has-error has-feedback' : ''}">
				...
				<c:if test="${practic != null }">
				<druzi:form-error path="items[${index }].responsibilities" />
				</c:if>
			</div>
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].demo"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<input class="form-control" name="items[${index }].demo" id="items${index }demo" 
										placeholder="Example: http://druzi.devstudy.net" value="${practic.demo }" >
				<c:if test="${practic != null }">
				<druzi:form-error path="items[${index }].demo" />
				</c:if>
			</div>
			<c:if test="${practic != null }"><druzi:form-has-error path="items[${index }].src"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<input class="form-control" name="items[${index }].src" id="items[${index }].src" 
										placeholder="Example: https://github.com/devstudy-net/druzi" value="${practic.src }" >
				<c:if test="${practic != null }">
				<druzi:form-error path="items[${index }].src" />
				</c:if>
			</div>
		</div>
	</div>
